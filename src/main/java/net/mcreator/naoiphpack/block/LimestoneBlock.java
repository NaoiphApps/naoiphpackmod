...
public class LimestoneBlock extends NaoiphPackElements.ModElement {
	@ObjectHolder("naoiphpack:limestone")
	public static final Block block = null;
	public LimestoneBlock(NaoiphPackElements instance) {
		super(instance, 135);
	}
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, Biome.createDecoratedFeature(new OreFeature(OreFeatureConfig::deserialize) {
				...
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				if (blockAt.getBlock() == LimeRockBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
				if (blockAt.getBlock() == LimeRockGrassBlock.block.getDefaultState().getBlock())
					blockCriteria = true;
