...
package net.mcreator.naoiphpack.world.dimension;
...
public class NaoiphAshDimensionDimension extends NaoiphPackElements.ModElement {
	@ObjectHolder("naoiphpack:naoiphashdimension")
	public static final ModDimension dimension = null;
	@ObjectHolder("naoiphpack:naoiphashdimension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public NaoiphAshDimensionDimension(NaoiphPackElements instance) {
		super(instance, 10);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("naoiphashdimension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("naoiphpack:naoiphashdimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("naoiphpack:naoiphashdimension"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("naoiphpack:naoiphashdimension"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("naoiphpack:naoiphslatebiome")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("naoiphpack:naoiphashbiome")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("naoiphpack:naoiphmaple")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("naoiphpack:oilocean")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new NaoiphAshDimensionItem().setRegistryName("naoiphashdimension"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("naoiphashdimension_portal");
		}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == NaoiphSlateBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == NaoiphSlateBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == NaoiphSlateBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos,
									flag ? NaoiphSlateBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, NaoiphSlateBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return NaoiphSlateBlock.block.getDefaultState();
				}
		...
		public BiomeProviderCustom(World world) {
			...
			for (Biome biome : this.biomes) {
				biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
					{
						carvableBlocks = ImmutableSet.of(NaoiphSlateBlock.block.getDefaultState().getBlock(),
