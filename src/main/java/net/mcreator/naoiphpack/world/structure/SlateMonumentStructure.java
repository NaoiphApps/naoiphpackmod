...
public class SlateMonumentStructure extends NaoiphPackElements.ModElement {
	public SlateMonumentStructure(NaoiphPackElements instance) {
		super(instance, 51);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 8000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("naoiphpack", "slatemonument"));
						if (template == null)
							return false;
