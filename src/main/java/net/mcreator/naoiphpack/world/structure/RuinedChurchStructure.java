...
public class RuinedChurchStructure extends NaoiphPackElements.ModElement {
	public RuinedChurchStructure(NaoiphPackElements instance) {
		super(instance, 73);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 7000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("naoiphpack", "ruinedchurch"));
						if (template == null)
							return false;
