...
public class PrisonStructure extends NaoiphPackElements.ModElement {
	public PrisonStructure(NaoiphPackElements instance) {
		super(instance, 38);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 8000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("naoiphpack", "prison"));
						if (template == null)
							return false;
