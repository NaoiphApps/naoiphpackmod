...
public class ChurchStructure extends NaoiphPackElements.ModElement {
	public ChurchStructure(NaoiphPackElements instance) {
		super(instance, 72);
	}
				...
				if ((random.nextInt(1000000) + 1) <= 4000) {
					...
					for (int a = 0; a < count; a++) {
						...
						Template template = ((ServerWorld) iworld.getWorld()).getSaveHandler().getStructureTemplateManager()
								.getTemplateDefaulted(new ResourceLocation("naoiphpack", "church"));
						if (template == null)
							return false;
