...
public class NaoiphAshBiomeBiome extends NaoiphPackElements.ModElement {
	@ObjectHolder("naoiphpack:naoiphashbiome")
	public static final CustomBiome biome = null;
	public NaoiphAshBiomeBiome(NaoiphPackElements instance) {
		super(instance, 9);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-52).waterFogColor(-52).surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(
							NaoiphAshBlock.block.getDefaultState(), NaoiphAshBlock.block.getDefaultState(), NaoiphAshBlock.block.getDefaultState())));
			setRegistryName("naoiphashbiome");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == NaoiphAshBlock.block.getDefaultState().getBlock() || ground == NaoiphAshBlock.block.getDefaultState().getBlock())
							&& (ground2 == NaoiphAshBlock.block.getDefaultState().getBlock()
									|| ground2 == NaoiphAshBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), NaoiphAshBlock.block.getDefaultState(), bbox);
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, NaoiphAshBlock.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												NaoiphSlateBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == NaoiphAshBlock.block.getDefaultState().getBlock()
					|| blockType == NaoiphSlateBlock.block.getDefaultState().getBlock()
					|| blockType == NaoiphAshBlock.block.getDefaultState().getBlock()
					|| blockType == NaoiphAshBlock.block.getDefaultState().getBlock();
		}
